...
def determine_best_offer(offers: list[dict[str, Service]]):
    best_offer = min(offers, key=lambda offer: offer.get("service").price)
    best_provider = best_offer.get("provider")
    return {"provider": best_provider, "service": best_offer.get("service").to_dict()}
...
class NextElement(State):
    async def run(self):
        ...
        logger.info(f"[Consumer {self.agent.jid}] Starting with the next element in recipe {self.agent.recipe}")
...
class Tender(State):
    async def on_start(self):
        self.offers = []
        logger.info(f"[Consumer {self.agent.jid}] Tendering for service {self.agent.current_recipe_element.get('service')}")

    async def run(self):
        requested_service = self.agent.current_recipe_element.get("service")

        providers = list(filter(lambda provider: requested_service.name in self.agent.providers.get(provider).get("services"), self.agent.providers.keys()))

        for provider in providers:
            send_message_behaviour = SendMessageBehaviour(
                provider, {"service": requested_service.to_dict()}, metadata={"performative": "call for proposal"}
            )
            self.agent.add_behaviour(send_message_behaviour)
            logger.info(f"[Consumer {self.agent.jid}] Sent query to {provider} for service {requested_service}")

            reply = await self.receive(timeout=5)
            if reply:
                logger.info(f"[Consumer {self.agent.jid}] Received reply from {provider}: {reply.body}")
                try:
                ...
                if reply.metadata.get("performative") == "propose":
                    if received_service.name == requested_service.name:
                        self.agent.current_recipe_element.get("providers").append(provider)
                        self.offers.append({"provider": provider, "service": received_service})
...
class SelectBestOffer(State):
    async def on_start(self):
        ...
        logger.info(f"[Consumer {self.agent.jid}] Selecting best offer for service {self.agent.current_recipe_element.get('service')}")

    async def run(self):
        offer = determine_best_offer(self.agent.offers)
        ...
        for provider in providers:
            if provider != self.best_provider:
                send_message_behaviour = SendMessageBehaviour(
                    provider, {"service": self.agent.current_recipe_element.get('service').to_dict()}, metadata={"performative": "reject proposal"}
                )
                self.agent.add_behaviour(send_message_behaviour)
    ...
    async def on_end(self):
        ...
        logger.info(f"[Consumer {self.agent.jid}] Best offer: {self.agent.best_offer} from {self.agent.best_provider}")


class BudgetCheck(State):
    async def on_start(self):
        logger.info(f"[Consumer {self.agent.jid}] Checking budget for service {self.agent.best_offer}")
    ...
    async def run(self):
        ...
        if budget_ok:
            send_message_behaviour = SendMessageBehaviour(
                self.agent.best_provider, {"service": self.agent.best_offer.to_dict()}, metadata={"performative": "accept proposal"}
            )
            self.agent.add_behaviour(send_message_behaviour)
            ...
            send_message_behaviour = SendMessageBehaviour(
                self.agent.best_provider, {"service": self.agent.best_offer.to_dict()}, metadata={"performative": "reject proposal"}
            )
            self.agent.add_behaviour(send_message_behaviour)
            self.set_next_state("LookForNewProvider")

    async def on_end(self):
        logger.info(f"[Consumer {self.agent.jid}] Budget check complete. Budget OK? {self.agent.budget >= self.agent.best_offer.price}")


class LookForNewProvider(State):
    async def on_start(self):
        logger.info(f"[Consumer {self.agent.jid}] Looking for new provider for service {self.agent.current_recipe_element.get('service')}")
...
class RequestService(State):
    async def on_start(self):
        logger.info(f"[Consumer {self.agent.jid}] Requesting service {self.agent.best_offer} from {self.agent.best_provider}")
        self.request_approved = False

    async def run(self):
        send_message_behaviour = SendMessageBehaviour(
            self.agent.best_provider, {"service": self.agent.best_offer.to_dict()}, metadata={"performative": "request"}
        )
        self.agent.add_behaviour(send_message_behaviour)
...
class WaitForService(State):
    async def on_start(self):
        logger.info(f"[Consumer {self.agent.jid}] Waiting for service {self.agent.best_offer} from {self.agent.best_provider}")
...
class ServiceComplete(State):
    async def run(self):
        ...
        send_message_behaviour = SendMessageBehaviour(
            self.agent.best_provider, {"service": self.agent.best_offer.to_dict(), "cost": self.agent.best_offer.price}, metadata={"performative": "inform"}
        )
        self.agent.add_behaviour(send_message_behaviour)
...
class CompleteRecipe(State):
    async def run(self):
        if self.agent.completed_recipes < 2:
            ...
            logger.info(f"[Consumer {self.agent.jid}] Recipe completed. New recipe: {self.agent.recipe}")
            print(f"[Consumer {self.agent.jid}] Starting with recipe: {self.agent.recipe} and budget: {self.agent.budget}")
            self.set_next_state("Idle")
        else:
...
def setup_FSM():
    fsm = FSMBehaviour()
    fsm.add_state(name="CheckOfferedServices", state=CheckOfferedServices(), initial=True)
